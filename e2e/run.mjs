...
async function main() {
  ...
  if (!SKIP_PROTOCOL) {
    ...
    const jsoloCode = await run(node, [path.join("e2e", "journey-solo-e2e.mjs"), BASE], { shell: false });
    const jmultiCode = await run(node, [path.join("e2e", "journey-multi-e2e.mjs"), BASE], { shell: false });
    ...
    protocolCode = lobbyCode || deckCode || saveCode || rosterCode || voteCode || claimCode || sandboxCode || stabilityCode || gameplayCode || abilityCode || chapterCode || traumaCode || clogCode || handCode || jsoloCode || jmultiCode || flowCode;   // 任一失敗即失敗
