...
rem
rem  用法:
rem    scripts\setup\setup-content.bat            (已有資料就略過)
rem    scripts\setup\setup-content.bat --refresh  (清快取重抓:出新擴充/改白名單後)
...
if /i "%~1"=="--refresh" (
  ...
  if exist "content\cards\generated\*.json" (
    echo ✓ 卡片資料已就緒。要更新請執行:scripts\setup\setup-content.bat --refresh
    pause >nul
    exit /b 0
  )
)
