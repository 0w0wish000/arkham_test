...
- 本遊戲**創傷(trauma)跨章保留**(✅ 已實作):章節結算依淘汰原因指派 —— 傷害被擊敗 +1 肉體創傷、恐懼被擊敗 +1 精神創傷(撤退無);下一章開局帶等量傷害/恐懼;創傷達生命/理智上限 → 該角色自動永久退役(進 `deadInvestigators`,玩家改帶新角色、創傷歸零)。創傷存於名冊與存檔,席位認領一併繼承。
- 觸發投票(可在死亡當下或下個牌組大廳):
```
...
## 11.5 跨劇本的牌組 / 角色變動(混合制)✅ 已實作(APPLY_LOG + 戰役日誌面板;見 docs/11 D6/D7)

劇本常在章節之間**強制改牌組或角色本質**(加弱點/故事卡、移卡、記旗標、創傷、劇情殺…)。
**原則:決定權在玩家(語音),記帳與同步由系統。**(貼合本作玩法:熟人自開語音、用本平台開區網共玩。)

- ✅ 系統**自動**處理「它算得出來的」:XP 結算(受上限,§11)、創傷跨章、永久卡(permanent)保留、死亡換角(§10)。
- ❌ 系統**不知道**的(劇本手冊敘事 —— FFG 版權、無結構資料)→ 提供「**套用劇本指示**」面板:一人對著語音念手冊,點選加卡 / 移卡 / 加弱點 / 記旗標 / 調創傷,系統套進**共享存檔** → 廣播全員 → 落地。**人決定、系統記帳**,不用手動、不會各記各的(這正是數位化的價值)。

| 劇本變動 | 自動? | 處理 |
|---|---|---|
| XP 結算 / 創傷 / 永久卡 / 死亡 | ✅ / ◐ | 系統自動(死亡走 §10 投票) |
| 加弱點·故事卡 / 移卡·封存 / 日誌旗標·密語·補給 / 角色轉變 | ❌ | 「套用劇本指示」面板(人套用、系統記錄 + 同步) |

- **觸發時機**:章節之間的**牌組大廳**(接 §9 跨章 + [docs/11 §D](11-engine-backlog.md))。
- **信任模型**:任一人套用即可,全員看得到旁白(例「劇本指示:每位調查員加入弱點〈某卡〉」);消耗性/爭議性項可加全員確認(沿用投票家族)。
- **協定(待做)**:`APPLY_LOG`(加/移卡、加弱點、記旗標、調創傷),於 DECKBUILDING 階段套用 → 廣播 `SESSION_ROSTER` + 寫存檔。**相依「跨章推進」** → 排在 docs/11 §D6 一起做。
