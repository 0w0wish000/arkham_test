...
| 身分 | 是什麼 | 存哪 |
|---|---|---|
| 🆔 **playerId** | 「人」的永久識別(首次輸入名稱,可改名);**換裝置認不回 → 入桌對離線席位按「🔑 這是我」由隊友同意繼承** | 各玩家本機 |
...
| 模組 | 狀態 |
|---|---|
| 規則引擎(官方行動全套:移動/調查/戰鬥/閃避/交戰/打牌/**啟動**/抽牌/資源/**撤退**/調整手牌/技能檢定/回合;個別淘汰制、趁隙攻擊、場上毀滅計數、**手牌上限自選棄牌**) | ✅ 可玩 |
| 大廳全套(docs/09 P1–P6:身分/建桌/名冊/牌組屏障/選卡器/存檔重載/中離·亂入·接手/死亡換角投票/**席位認領**) | ✅ |
| 戰役系統(跨章推進 + XP 結算、**創傷跨章**、**戰役日誌/劇本指示面板**、章節感知劇本檔) | ✅ lite |
| 場景資料化(劇本 = JSON:地點圖/幕密謀**牌組**/敵人/遭遇;遭遇洗牌可重現) | ✅ lite |
| 效果 DSL(打出 / 常駐修正 / 啟動 三張資料表 + 引擎直譯;= 卡牌資料 effects 目標格式) | ✅ 垂直切片 |
| 能力時機引擎(反應詢問/強制/檢定窗口)| ◐ 原型(完整多人窗口待 B3) |
| 全卡實作 / 戰役劇情文本 | ⏳ 待卡牌資料定案 / IP(見 [docs/11](11-engine-backlog.md)) |

> 驗證:**引擎單元測試 50 個 + e2e 15 套 184 斷言**(`node e2e/run.mjs` 一鍵全跑;e2e 用專用埠 **18080**,不佔用正式的 8080)。
