...
Round:
  ...
  2. Investigation Phase
       - 玩家**自選順序**(合作決定誰先行動),一次一位調查員進行其 3 個行動;**非固定輪替**(見 §4.1)
  3. Enemy Phase
...
Arkham 是**合作**遊戲,順序刻意保持彈性,**不強制固定排序**:

- **調查階段行動順序 = 玩家自選(p.11):** 團隊自行決定誰先進行整個回合,結束後再選下一位;**不是固定輪替**,也不規定誰先打什麼牌。引擎/大廳只追蹤「本輪尚未行動的調查員」,任一位皆可接手。
...
> **🔧 引擎/伺服器:** 不要硬編固定回合順序。`activeInvestigatorId` 由團隊指定 / 自願接手;commit 窗以**到達順序**處理、驗證後即鎖定。只有 mandatory 同時步驟才套座位順序。
